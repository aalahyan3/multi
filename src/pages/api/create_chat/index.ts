...
export default async function handler(req: NextApiRequest, res: NextApiResponse){
    ...
    console.log(curr_user, req.body);
    

    if (!with_user || !curr_user)
        return res.status(405).json(ApiRespBuilder(false, "can't perform action", 500, null));
    ...
    if (chat)
    {
        return res.status(200).json(ApiRespBuilder(true, "chat was created",200,  chat));
    }
